...
class RPC
{
    ...
    {
        if (!is_array($ids)) $ids = array($ids);    // Convert $ids to an array if only a single id was passed
        if (count($fields) == 0) $fields = array("id", "name", "status", "addedDate", "haveValid", "totalSize", "sizeWhenDone");    // Defaults
        $request = array(
            "fields" => $fields,
            "ids" => $ids
        );
    ...
    public function rename($ids, $path, $name)
    {
        ...
        if (count($ids) !== 1) {
            throw new RPCException('A single id is accepted', RPCException::E_INVALIDARG);
        }
